...
const Register = () => {
  ...
  useEffect(() => {
    ...
    // Prefill access code from URL (worker invite flow)
  ...
  const handleSubmit = async (e) => {
    ...
    let requestFn = authAPI.register; // default: USER

    if (role === "WORKER") {
      if (!access_code.trim())
        return toast.error("Access code is required for workers.");
      if (!worker_category_id)
        return toast.error("Worker category ID is required.");
      payload.access_code = access_code;
      payload.worker_category_id = worker_category_id;
      requestFn = authAPI.internalRegister;
    } else if (role === "VENDOR") {
      if (!company_name.trim())
        return toast.error("Company name is required for vendors.");
      payload.company_name = company_name;
      requestFn = authAPI.registerVendor;
    } else if (role === "PARTNER") {
      if (!agency_name.trim())
        return toast.error("Agency name is required for partners.");
    ...
    try {
      ...
      navigate(
        `/verify-otp?email=${encodeURIComponent(
          returnedEmail
        )}&phone=${encodeURIComponent(returnedPhone)}`
      );
    } catch (err) {
  ...
  const handleGoogleSuccess = async ({ credential }) => {
    if (!credential)
      return toast.error("Google login failed: Missing credential");
    setLoading(true);
    try {
