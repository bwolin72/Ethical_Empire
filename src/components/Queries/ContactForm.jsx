...
const contactInfo = {
  ...
  socialLinks: [
    { name: 'Instagram', url: 'https://www.instagram.com/ethicalmultimedia' },
    { name: 'LinkedIn', url: 'https://www.linkedin.com/in/ethical-empire/' },
    { name: 'Twitter', url: 'https://x.com/EeTHm_Gh' },
    { name: 'Facebook', url: 'https://facebook.com/16nQGbE7Zk/' },
  ],
};
...
const ContactForm = () => {
  ...
  const validateClient = () => {
    ...
    if (formData.event_date) {
      ...
      if (new Date(formData.event_date) < today) newErrors.event_date = 'Event date cannot be in the past.';
    }
    setErrors(newErrors);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.status === 201) {
        ...
        setFormData({ name:'', email:'', phone:'', country:'', region:'', enquiry_type:'', service_type:'', event_date:'', description:'' });
      }
    } catch (err) {
      ...
      <main className="contact-grid">
        <section className="contact-form-section animate-fade-in-up">
          <form onSubmit={handleSubmit} noValidate>
            ...
            <div className="form-group full-width">
              <textarea name="description" rows="4" value={formData.description} onChange={handleChange} placeholder=" " />
              <label>Message</label>
            </div>
        ...
        <aside className="company-details-section animate-fade-in-up">
          ...
          <div className="contact-buttons">
            <a href={`https://wa.me/${contactInfo.coFounder.whatsapp.replace(/\D/g,'')}`} className="whatsapp">WhatsApp</a>
            <a href={`tel:${contactInfo.coFounder.phone.replace(/\D/g,'')}`} className="phone">Call</a>
          </div>
