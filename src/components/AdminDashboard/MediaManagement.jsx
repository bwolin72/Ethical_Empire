...
const MediaManagement = () => {
  ...
  const handleUpload = async () => {
    if (!files.length)
      return toast.warn("Select at least one file.", { autoClose: 3000 });
    setUploading(true);
    setUploadProgress(0);
  ...
  const toggleFeatured = async (id) => {
    try {
      ...
      toast.info(
        data.is_featured ? "Marked as featured." : "Unmarked.",
        { autoClose: 2500 }
      );
      setUploadedItems((prev) =>
        prev.map((item) =>
  ...
  return (
    <div className="admin-dashboard-preview">
      ...
      >
        <SortableContext
          items={uploadedItems.map((item) => item.id)}
