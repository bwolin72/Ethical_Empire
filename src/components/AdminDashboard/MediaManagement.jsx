...
const MediaManagement = () => {
  ...
  const fetchMedia = useCallback(async () => {
    try {
      ...
      else if (typeof mediaAPI[endpointKey] === "function") res = await mediaAPI[endpointKey]();
      else res = await mediaAPI.list({ endpoint: endpointKey });
  ...
  const handleUpload = async () => {
    ...
    try {
      await mediaAPI.upload(files, { type: mediaType, label: label || "Untitled", endpoint: selectedEndpoints });
      fetchMedia(); setFiles([]); setLabel("");
  ...
  const handleFileChange = (e) => {
    ...
    const invalidFiles = selected.filter(file => !ACCEPTED_FILE_TYPES.some(type => file.type.startsWith(type)) || file.size > MAX_FILE_SIZE_MB * 1024 * 1024);
    if (invalidFiles.length) {
      invalidFiles.forEach(file => toast.warn(`${file.name} ${file.size > MAX_FILE_SIZE_MB*1024*1024 ? "exceeds 10MB limit" : "not a valid image/video"}`, { autoClose: 5000 }));
      return;
    }
    setFiles(selected);
  };
  ...
  return (
    <div className="admin-dashboard-preview">
      ...
      <div className="media-controls">
        ...
        <button onClick={handleUpload} disabled={uploading || files.length === 0}>{uploading ? `Uploading ${uploadProgress}%...` : "Upload"}</button>
        <button onClick={() => setMediaType(prev => prev === "media" ? "banner" : prev === "banner" ? "featured" : "media")}>Switch to {mediaType === "media" ? "Banner" : mediaType === "banner" ? "Featured" : "Media"}</button>
      </div>
      ...
      <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
        <SortableContext items={uploadedItems.map(item => item.id)} strategy={verticalListSortingStrategy}>
          <div className="media-list">
            ...
            {uploadedItems.filter(item => item.label?.toLowerCase().includes(searchQuery.toLowerCase()) || item.url?.full?.toLowerCase().includes(searchQuery.toLowerCase()))
              .map(item => <SortableMediaCard key={item.id} item={item} toggleActive={toggleActive} toggleFeatured={toggleFeatured} deleteMedia={deleteMedia} setPreviewItem={setPreviewItem} />)}
          </div>
        </SortableContext>
      </DndContext>
