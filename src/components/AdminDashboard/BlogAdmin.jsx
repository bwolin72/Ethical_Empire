    ...
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <section className="mb-10">
        ...
        <form onSubmit={handleCategorySubmit} className="mb-4 flex gap-2">
          <input type="text" value={categoryForm.name} onChange={handleCategoryChange} placeholder="Category Name" className="p-2 border rounded-lg flex-1" required />
          <Button type="submit"><Save className="w-4 h-4 mr-1" />{editingCategory ? "Update" : "Add"}</Button>
        </form>
          ...
          {categories.map((cat) => (
            <Card key={cat.id} className="p-4 flex justify-between items-center">
              ...
              <div className="flex gap-2">
                <Button onClick={() => handleEditCategory(cat)} variant="outline"><Edit className="w-4 h-4" /></Button>
                <Button onClick={() => handleDeleteCategory(cat.slug)} variant="destructive"><Trash className="w-4 h-4" /></Button>
              </div>
            </Card>
          ))}
        </div>
      ...
      <section className="mb-10">
        ...
        <form onSubmit={handlePostSubmit} className="mb-6 grid gap-4">
          ...
          <Button type="submit"><Save className="w-4 h-4 mr-1" />{editingPost ? "Update Post" : "Add Post"}</Button>
        </form>
